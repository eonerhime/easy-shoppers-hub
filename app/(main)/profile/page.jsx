...
export default function ProfilePage() {
  ...
  // Load user's session and order details from the DB
  useEffect(() => {
      ...
      try {
        ...
        if (orders) {
          ...
          data.forEach((order) => {
            ...
            // Define current year here
            const currentYear = new Date().getFullYear();

            // Define current month here
            const currentMonth = new Date().getMonth() + 1;

            // Lifetime
            lifetimeOrders += 1;

            lifetimeSpent += totalAmount;

            // Yearly
            if (orderYear === currentYear) {
              yearlyOrders += 1;

              yearlySpent += totalAmount;
            }
